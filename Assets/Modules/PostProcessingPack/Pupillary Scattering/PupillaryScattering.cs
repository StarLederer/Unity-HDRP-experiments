...
public sealed class PupillaryScattering : CustomPostProcessVolumeComponent, IPostProcessComponent
{
	// Effect settings
	...
	[Header("Scattering")]
	public ClampedFloatParameter lenticularHaloRadius = new ClampedFloatParameter(64f, 0f, 512f);
	public ClampedFloatParameter lenticularHaloThicnkess = new ClampedFloatParameter(30f, 1f, 300f);
	public ClampedIntParameter lenticularHaloBladeCount = new ClampedIntParameter(20, 2, 100);
	...
	// Just yoinked, don't fully understand what this is used for
	// Probably need to got rid of it
	...
	unsafe void DoBloom(CommandBuffer cmd, HDCamera camera, RTHandle source, Material material)
	{
		...
		// Lenticular halo
		RTHandle haloTexture;
		switch (resolution)
		{
			case BloomResolution.Half:
				haloTexture = m_Pool.Get(new Vector2(0.5f, 0.5f), k_ColorFormat);
				break;
			default:
				haloTexture = m_Pool.Get(new Vector2(0.25f, 0.25f), k_ColorFormat);
				break;
		}
		...
		cmd.SetComputeTextureParam(cs, kernel, Shader.PropertyToID("_InputHaloTexture"), haloTexture);
		cmd.SetComputeFloatParam(cs, Shader.PropertyToID("_LenticularHaloIntensity"), lenticularHaloIntensity.value);
		cmd.SetComputeIntParam(cs, Shader.PropertyToID("_LenticularHaloBladeCount"), lenticularHaloBladeCount.value);
		cmd.SetComputeTextureParam(cs, kernel, Shader.PropertyToID("_OutputTexture"), m_BloomTexture);
		cmd.SetComputeVectorParam(cs, Shader.PropertyToID("_BloomBicubicParams"), new Vector4(lowSize.x, lowSize.y, 1f / lowSize.x, 1f / lowSize.y));
		cmd.SetComputeVectorParam(cs, Shader.PropertyToID("_TexelSize"), new Vector4(highSize.x, highSize.y, 1f / highSize.x, 1f / highSize.y));
		DispatchWithGuardBands(cs, kernel, highSize);

		m_Pool.Recycle(haloTexture);
		haloTexture = null;

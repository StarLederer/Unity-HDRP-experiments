Shader "Hidden/Shader/Bloom"
{
    ...
    float4 HorizontalBlur(Varyings input) : SV_Target
    {
		...
		for(int i = 1; i < _Steps; i++)
		{
	        ...
	        uint2 positionSSleft = uint2(positionSS.x - i, positionSS.y);
	        outColor += LOAD_TEXTURE2D_X(_InputTexture, positionSSleft).xyz * weight;
	        totalWeight += weight;

	        uint2 positionSSright = uint2(positionSS.x + i, positionSS.y);
	        outColor += LOAD_TEXTURE2D_X(_InputTexture, positionSSright).xyz * weight;
	        totalWeight += weight;
		}
